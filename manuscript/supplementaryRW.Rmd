...
plots in \"Simulation length and equilibrium criteria\"). These stem
from 10 year periods that overlap partially (for \< 2500 simulation
...
not already died until the sample year. The recruit's basal area (r.ba)
is the sum of the basal area per hectare of the trees that cross the
diameter threshold that have not already died until the sample year. The
total stand basal area (ba) is the basal area per hectare of all trees
in the stand \> 0 cm diameter at breast height (no lower boundary) at
the sample time per species. We provide the data for both diameter
...
We use the LandClim version 2.0, which calculates forest dynamics at a
decadal time step. The complete reference resource for the LandClim
model and regeneration module are:
...
sampled the climate time series randomly.

### Soil

We used the soil quality provided to infer the Available Water Holding
Capacity needed in LandClim. We assigned 8 cm for soil quality one and
25 cm for soil quality five and interpolated the values for the other
categories linearly between the two extremes.

### Topography

We used the altitude and soil information provided in the protocol to
create the different landscapes, one per site. However, each landscape
created per site had no slope, altitude, or aspect variation.

### Tree species

We simulated forest dynamics only considering the 11 species proposed in
...
We simulate forest dynamics from bare ground for each of the sites. We
prepared landscapes of 250x250m without topographical elevation for each
of the sites. The simulations we run from bare ground to 2000 years and
the samples were taken from the year 2000. On the regeneration module,
we use a background probability of 0.001, a dispersal mode based on the
formulation random asymmetric, and the same establishment probability
for all the species included in the simulation of 0.9.

### Outputs

We estimated the outputs based on a sample of 25 random cells every ten
years from 2000 to 4000 of the simulation for one simulation per site,
providing 200 samples per site.
