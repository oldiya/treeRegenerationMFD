...
across Europe. The simulation results are compared with observed data
...
levels, species composition and mortality in tree recruitment. The
results show that most of the models of forest dynamics tend to
over-estimate recruitment rates although this is not always connected to
large mortality rates in early dynamics stages. The effect of light
availability on recruitment rates is better captured than temperature or
soil moisture, however the patterns are not consistent across models.
Species richness matches quite well compared to the ranges observed in
the data. Models simulating higher species diversity at the stand level
do not feature higher diversity in recruitment. *Abies alba* was well
captured across temperature and soil moisture gradients by most models
and *Fagus sylvatica* was generally underestimated. *Picea abies*
recruitment was generally over-estimated on colder sites. HERE TWO
SENCENTECES DISCUSSION/CONCLUSION. HERE A SO WHAT SENTENCE
...
[@bogdziewicz2021]. Finding the right level of complexity in the
definition of the factors leading to tree recruitment is challenging and
models of forest dynamics have used different approaches [@bugmann2022].
Overall, models are needed to (1) synthesize existing empirical data and
explore their consequences, and (2) assess future tree recruitment e.g.
in the context of global change scenarios. Currently, regeneration
processes in dynamic forest models are handled in a multitude of ways
[@konig_tree_2022; @bugmann2022], from entirely ignoring tree
regeneration (as done in classical forest growth models, e.g.
@pretzsch2002), across the use of a few simple environmental filters, as
done in most forest "gap" models (e.g., @shuman2011) and Dynamic Global
Vegetation Models (e.g., @smith2001; @hickler2012), to complex
approaches that incorporate local feedback from the canopy, multiple
ecological processes and often also small time steps (e.g., @wehrli2006;
@seidl2012), or field-based statistical parameterizations, which however
are not easy to extrapolate in space and time (e.g., @ribbens1994).
...
problems and have strong impacts on forest dynamic such as
over-recruitment, needed excess of mortality at early stages and
inadecuate species composition in regeneration. Some model outputs are
...
undesired behaviors in tree recruitment are often not published, but in
the vast majority of studies they are corrected (e.g., @rasche2011,
where for most study sites regeneration was simply switched off). Thus,
a strong focus on the modeling of tree regeneration processes is highly
important and long overdue [@price] if we are to make reliable
...
Tree recruitment is understudied in field studies and modelling
[@hanburybrown_forest_2022], as little data has been available and it is
often very fragmented, i.e., not in one harmonized data set. On the one
...
after the stand initiation phase on productive forests [@hallsby2015].
Data from permanent plots, although available across regions, it is also
highly heterogeneous in design making their usability more difficult
[@käber2021]. On the other hand, targeted studies to measure tree
recruitment on experimental sites are often limited in space, species or
the relevant processes leading to regeneration that are targeted
[@berdanier2016; @collet2006].
...
[@käber2022]. In this study we relied on models with various
'philosophies', including models operating at the stand to the global
scale. Because of the large variability in recruitment patterns in
nature and the large amount of factor driving recruitment, including
some which are not included in most models, such as deer browsing, we do
not aim at a detailed statistical evaluation of each model. Instead, we
aim at evaluating general recruitment patterns, magnitudes and benchmark
the regeneration niche of multiple species based on empirical data along
a wide environmental gradient (temperature, moisture, and light
...
HERE **FORCLIM** WRITE 2 SENTENCES ON THE DIFFERENCES BETWEEN FORCLIM 1
AND 11 AND WHY IT IS RELEVANT TO CONSIDER THE TWO MODEL VERSIONS. HERE
**FORCEEPS** WRITE 2 SENTENCES ON THE DIFFERENCES BETWEEN FORCEEPS AND
FORCEEPS(F) AND WHY IT IS RELEVANT TO CONSIDER THE TWO MODEL VERSIONS.
...
Each modeler group was provided with a detailed protocol (available in
the supplementary material) with instruction on how to perform the
...
supplementary information / repository.
...
in basal area as for a same sample recruitment at 10 cm will include
...
capturing regeneration in forest dynamics is fundamental to for example
evaluate post-disturbance dynamics as this will define the status of the
forests in the first 15-20 years [@seidl_post-disturbance_2022]. If this
initial state it is not well captured then we can not properly assess
aspects such as the forest resilience, or changes in carbon capture. In
this study, for the first time, projections of tree recruitment done by
several models of forest dynamics were confronted with observed data
across a large environmental gradients.

The observed data that we used [@käber2022] is exceptional in this
regard, and to use it for model benchmarking has gave us insights on
recruitment levels, species composition and mortality and overall a
broader understanding than if we have used these data for a model
calibration. The lower available threshold in the observed data of 7 cm
provides a limit from which we can understand tree recruitment, which
might be a rather long period since germination as relevant
environmental constraint might have already act on those young trees
[@käber2021]. However recruitment data with lower thresholds are not
available in a harmonized manner from large environmental gradients. The
observed data was collected from forest reserves where there has not
been management for long periods of time, this might help to properly
assume that these structures are in an equilibrium state. This
assumption might also justify some of the rather large recruitment rates
that we are seeing in the models projections compare to the observed
ones.

Adequate tree recruitment rates define the structure and composition of
forests. Most of the models of forest dynamics tended to over-recruit
trees in all the sites. Over estimating recruitment might have
fundamental implications on the role of regrowth and forest carbon, with
an overestimation in their capacity for carbon capture for example after
the impact of disturbances [@pugh2019]. Excess in recruitment also
requires of larger mortality rates in later stages, changing the
expected forest densities at early stages and forcing the models to have
stronger filters. Regeneration levels define the success of future
forests, but it is equally important to correctly identify un-stocked
forest areas [@rammer2021]. Tree recruitment is often characterized by
an excess number of zero counts [@fortin2007]. In this study some of the
models did not have lack of recruitment or their proportion was very
low. This substantial difference with the observed data might also be a
consequence of assuming that the observed data is also in equilibrium,
where lack of recruitment in sites could also be an indication of
"non-equilibrium" due to management history.

Light availability effect on tree regeneration have been widely
documented [@messier1999; @collet2006; @berdanier2016]. However it is
difficult to collect variables defining light availability on
regeneration. In this study, we used the total stand basal area as a
proxy for light availability. There was differences in the basal area
ranges, some of the models had stand basal areas lower than the observed
data but there was also models that did have more stocked stands than
the observed data, therefore it was not possible to evaluate the
recruitment for the extremes of the density ranges in some models.
Regeneration at low stand densities is interesting to assess how well
forests are recovering e.g. after the creation of gaps due to
disturbances. We would have expected that the recruitment of the
different species would become sparser and droped out completely as a
function of a proxy of light availability, however few models showed
this trend indicating that the relation between recruitment and light
availability it is not yet correctly captured.

Correctly capturing tree composition in recruitment help us to assess
the functional composition of the forest appropriately, e.g. their
sensitive to drought or resilience to new disturbances events
[@redmond2015; @seidl_post-disturbance_2022]. In the models simulation
outputs overall species richness levels in recruitment was
underestimated but inside the observed ranges, however there were
variations on how the main tree species were represented in the
environmental gradient. The lower species richness in recruitment might
be a consequence of models running without disturbances, disturbances
would typically allowed for larger gaps sizes allowing less shade
tolerant species to grow in larger proportions. Most models maintain or
even increase in recruitment diversity between the 7 and 10 cm
thresholds, and the same is visible from the observed data, however the
differences are not significant. Species richness variations might be
explained by the way each model considers recruitment, e.g. in the
absence of adults, recruitment can be linked to either seed
rain-dispersal, or to seed/seedling bank, but also on what processes are
formulated with stochastic or deterministic approach. Differences might
also be expected between cohort and individual trees based models
**how?**. However, our results do not show differences between models
with and without feedback in recruitment, **why?** The adult forests
species richness was better captured than the recruitment species
richness, and only few models over-predicted stand level species
richness **why these ones?**.

We could expect that models that over-recruit trees will also have a
large mortality between 7 and 10 cm thresholds but this was not shown.
This implies that these models must have the excess of mortality in
later stages. Higher mortality in the adult component might enhance the
share of less shade tolerant species for those models. There might be a
difference between models including stochastic processes in recruitment
and those which do not **??**. There was also a group of models below
the Reineke line indicating a lower mortality than expected by the
self-thinning rule, which may point to facilitation, or simply a higher
mortality rate before the trees reached 7 cm diameter, another
possibility would have been that these models underestimate recruitment
which was not the case. This might also be partly an artifact of the
method, as we are expecting space for time substitution, but in reality
we just have one measurement.

There is also the question of how much level of detail one have to give
...
the species level parametrization. **How this relates to the results?
Future model development recommendations?**

In these study we considered a wide range of models of forest dynamics,
from stand to global scale. However, the simulation set up was limited
to 1 ha recruitment values. It is important to include spatial
considerations on tree recruitment [@beckage2003] and the lack of it
might represent a more important disadvantage for landscape level
models. Without spatial context we are limiting recruitment to the seed
influx from the stand itself if the model does not have a background
seed input. But even global vegetation models are lacking this dispersal
between cells where there is an abstraction of the space or even the
trees themselves [@hanburybrown_forest_2022]. The selected models also
had limitations on species representation, some models where not able to
simulate certain species due to lack of species parametrization. We also
had one model that simulate PFTs providing a less flexible comparison
the with species values; with the lose of the species specific
information might also represent substantial ecological
representativeness drawbacks [@purves2008].

**Conclusion paragraph**

Capture properly recruitment rates, species composition and mortality in
the environmental gradient might impact future forests...
