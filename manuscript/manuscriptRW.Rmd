...
these 200 sites. We aim to assess how well the models of forest dynamics
are capturing the overall abundance, species composition and mortality
in tree recruitment. The results show that most of the models tend to
...
higher diversity in recruitment. *Abies alba* was well captured across
temperature and soil moisture gradients by most models, whereas *Fagus
sylvatica* was generally underestimated. *Picea abies* recruitment was
generally over-estimated, particularly at colder sites. HERE TWO
...
underlying forest gap models (ForClim 1, ForClim 11, LandClim, Landis-II
); whereas some include plant ecophysiological processes (FORMIND,
...
Table S2), and the same was visible from the observed data. There are
only few models with large recruitment diversity increasing between 7
and 10 cm (ForCEEPS, TreeMig, LPJ-GUESS).
...
effect of the number of individuals at 7cm on the mortality rate. Most
of the models show a positive effect (increasing recruitment at 7cm
shows increases the mortality between 7 and 10cm). Some models show a
...
approach. Differences might also be expected between cohort and
individual trees based models. MAKE REFERENCES TO LITERATURE

Our results do not show differences between models with and without
feedback in recruitment. The adult forests species richness was better
captured than the recruitment species richness, and only few models
overpredicted stand level species richness. Most of the models have
feedback on recruitment from the adult trees, therefore EXPAND ON
FEEDBACK ISSUE

## Mortality in regeneration

As mentioned above, it is reasonable to expect that models that
over-recruit trees will also have a couting-acting high mortality
between the 7 and 10cm thresholds, but this was nit cinsistently evident
from the simulations. This implies that these models must have the
excess of mortality in earlier or later stages. Higher mortality in the
adult component might enhance the share of less shade tolerant species
for those models. There might be a difference between models including
stochastic processes in recruitment and those which do not. There was
also a group of models below the Reineke line indicating a lower
mortality than expected by the self-thinning rule, which may point to
facilitation, or simply a higher mortality rate before the trees reached
7 cm diameter, another possibility would have been that these models
underestimate recruitment which was not the case. This might also be
partly an artifact of the method, as we are expecting space for time
substitution, but in reality we just have one measurement. NEED MORE
REFERENCES AND DISCUSSION POINTS
...
In this study, we considered a wide range of models of forest dynamics,
...
might represent an more important disadvantage for landscape level
...
trees themselves [@hanburybrown_forest_2022]. Landscape level models
have been build to be very accurately at the landscape level. EXPAND ON
THE DIFFERENT SCALE LEVELS AND IMPLICATIONS

Modeling approach seems more important than scale and accuracy of the
design. Model behavior (accuracy) is completely unrelated to model scale
(stand, landscape, global) and approach (empirical, process-based).
Having both empricial and process based models, the expectation was that
the empirically based models (SIBYLA, xComp) would have a better
performance as they are calibrated with inventory data, however this was
not the case. EXPAND ON

The selected models also had limitations on species representation, some
models where not able to simulate certain species due to lack of species
parametrization. We also had one model that simulate PFTs providing a
less flexible comparison the with species values; with the lose of the
species specific information might also represent substantial ecological
...
Cautious when using this models to evaluate regeneration post
disturbances we might be overestimation their capacity for carbon
capture
...
Provide succinct answers to the three questions/objectives stated at the
end of the
Introduction. Perhaps end with a concluding (\"more positive\") final
paragraph.

Models eventually get the right stand structure and composition, most of
them do that
by compensating for one error (overestimation of recruitment) by another
error
(excessive subsequent mortality, if not between 7 and 10 cm, then
shortly
thereafter): \"getting the right answer for the wrong reason\". And this
is
worrysome in general, but particularly (and very acutely) if the models
are to
be used e.g. under changing climatic conditions.
