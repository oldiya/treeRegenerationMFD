...
done in most forest \"gap\" models (e.g., @shuman2011) and Dynamic
Global Vegetation Models (e.g., @smith2001; @hickler2012), to complex
...
capturing regeneration in forest dynamics is fundamental to evaluate for
example post-disturbance dynamics as this will define the status of the
...
multitude models of forest dynamics were confronted with observed data
across a large environmental gradients. The observed data that we used
[@käber2022] is exceptional in this regard, and that's why it was used
for model benchmarking rather than calibration.
...
trees in all the sites. Excess in recruitment has to lead to a large
mortality in later stages. Capture properly recruitment rates in the
environmental gradient might impact future forests. If not capture well
it is going to be difficult to for models to correctly project range
shifts retractions in species expansions because of climate change might
not also be properly represented if we do not capture properly
recruitment rates in the environmental gradient.

Regeneration levels define the success of future forests, but it is
equally important to correctly identify un-stocked forest areas
[@rammer2021]. Tree recruitment is often characterized by an excess
number of zero counts [@fortin2007]. In this study some of the models
did not have lack of recruitment or their proportion was very low. This
substantial difference with the observed data might be a consequence of
assuming that the observed data is also in equilibrium, where lack of
recruitment in sites could also be an indication of "non-equilibrium"
due to management history.

Light availability is an important process defining tree recruitment
[@collet2006; @berdanier2016]. However it is difficult to have measured
variables defining light availability for the regeneration. In this
study we used the total stand basal area as a proxy for light
availability. There was differences in the basal area ranges, some of
the models had stand basal area lower than the observe data but there
was also models that did have more stocked stands that the observed data
and therefore it was not possible to evaluate the recruitment for lower
density stands. It was also not possible to observed the expected
pattern showing how recruitment of the different species is becoming
sparser and drops out completely as a function of a proxy of light
availability.

If the recruitment tree composition is not well captured then we can not
assess the functional composition of the forest appropriately, e.g.
their sensitive to drought or resilience to new disturbances events. In
the models simulation we saw variations on how the main tree species
were represented in the environmental gradient but also in the in
overall species richness levels. Most models maintain or even increase
in recruitment diversity between the 7 and 10 cm thresholds, and the
same is visible from the observed data, however the differences are not
significant. Species richness variations might be explained by the way
each model considers recruitment, e.g. in the absence of adults
recruitment can be linked to either seed rain from elsewhere -dispersal,
or to seed/seedling bank). Differences might also be expected between
...
-   Is the adult forest the right forest, how is this relevant and is
    ...
    is highly dependent on the model structure:Background seed Yes/no /
    Stochastic vs deterministic

-   Why the models over predicting species richness at the stand level
    are all the "ForClim family" and FORMIND ?

We could expect that models that over-recruited trees. will also have a
large mortality between 7 and 10 cm thresholds. This implies that this
models have to have the excess of mortality in later stages of the
forest dynamics. Models below the Reineke line have lower mortality than
expected by the self-thinning rule, which may point to facilitation, or
simply a higher mortality rate before the trees reached 7 cm diameter,
however one would also assume that these models will underestimate
recruitment which was not the case. Why?. There might be a difference
between models including stochastic processes in recruitment and those
which do not, but perhaps the reality implies higher mortality rates in
the adult component (enhance the share of less shade tolerant species).
...
-   Recruitment of trees into size classes of ≥4 cm DBH, as in our four
    data sets, represents a relatively late stage of forest
