...
show that most of the models of forest dynamics tend to over-estimate
recruitment rates although this is not always connected to large
mortality rates in early dynamics stages. The effect of light
...
across a large environmental gradients. The observed data that we used
[@käber2022] is exceptional in this regard, and to use it for model
benchmarking has gave us much more insights than if we have used it for
calibration.
...
low. This substantial difference with the observed data might be a
...
difficult to have variables defining light availability for the
...
recruitment for lower density stands. Regeneration at low stand
densities is an interesting aspect to evaluate how well forests are
recovering e.g. after the creation of gaps due to disturbances. We would
have expected that the recruitment of the different species would become
sparser and droped out completely as a function of a proxy of light
availability, however few models showed this trend indicating that the
relation between recruitment and light availability it is not yet
correctly defined.

When tree composition in recruitment is not well captured, then we can
not assess the functional composition of the forest appropriately, e.g.
their sensitive to drought or resilience to new disturbances events
[@redmond2015; @seidl_post-disturbance_2022]. In the models simulation
outputs overall species richness levels in recruitment were inside the
observed ranges, however there were variations on how the main tree
species were represented in the environmental gradient. Most models
maintain or even increase in recruitment diversity between the 7 and 10
cm thresholds, and the same is visible from the observed data, however
the differences are not significant. Species richness variations might
be explained by the way each model considers recruitment, e.g. in the
...
with an without feedback in recruitment, **why?** The adult forests
...
and those which do not ??. There was also a group of models below the
Reineke line indicating a lower mortality than expected by the
...
the species level parametrization. **How it relates to the results?
...
-   We use all the models to rpoduce values per 1ha, stand ang global
    models potentially disregarding the advantages that landscape models
    present with spatial seed dispersal (TreMig, iLand and LAndClim
    references). Without spatial context we are limited to the seed
    influx from the stand itself if the model does not have a background
    seed input. But even global vegetation models are lacking this
    dispersal between cells [@hanburybrown_forest_2022] abstraction of
    space or even the trees themselves. It is important to include
    spatial considerations on tree recruitmen [@beckage2003]

-   Comparing PFTs with species. We lose the species specific
    information on with substantial drawbacks [@purves2008]

-   The loss of species-specific environmental responses through the
    collation of species communities into PFTs has been widely accepted,
    as parameterization efforts would otherwise exceed the available
    means. However, it is doubtful whether the definition of PFTs
    provides sufficient flexibility for an adequate representation of
    the ecological processes and the differences between species

-   Recruitment of trees into size classes of larger 4 cm DBH, as in our
    four data sets, represents a relatively late stage of forest
    regeneration. It integrates over sometimes long periods since
    germination and multiple environmental constraints act upon young
    trees of widely different size. kaber 2021
...
Capture properly recruitment rates in the environmental gradient might
impact future forests.
